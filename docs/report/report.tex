...
\bigskip
All the SIMD instructions have a name of the form:
\begin{center} 
...
\begin{remark}
    The \texttt{size} part usually refers to the size of the input but in some cases, it can contain both the
...
The compiler can use SIMD instructions when asked with the corresponding
optimization flags. In the case of \texttt{gcc}, one can use the
\texttt{-O2} and \texttt{-ftree-vectorize}
flags\footnote{\url{https://gcc.gnu.org/onlinedocs/gcc/Optimize-Options.html}}.
Conversely, the auto-vectorization of \texttt{gcc} can be disabled using the \texttt{-fno-tree-vectorize} flag.

In our timing measurements, we wanted a high level of optimization for our implementations but no vectorization
of any kind on the sequential versions. This can be done by using the flag \texttt{-O3} and by adding
to each sequential function \texttt{\_\_attribute\_\_((optimize("-fno-tree-vectorize")))}.


\bigskip
To help the compiler vectorize the code of a loop, we used the loop unrolling technique
where the body of the loop is replicated $k$ times and the increment step is set to $k$
which can sometimes increase the program efficiency.
...
the multiple load and store operations, and the memory issues that can arise from the size of the caches.
...
\begin{remark}
    ...
    For example, the Zen 4 processor has a clock speed of 3.3 GHz which means that it can handle about 
...
since they correspond to the number of pairs of coefficients they can handle simultaneously.
...
\begin{remark}
    ...
    This is the approach we used as it significantly speeds up the \texttt{mulhi} operation.
\end{remark}
...
It allowed us to highlight the contrasts between the different methods of vectorization and measure the performances
...
Thus, to circumvent its high cost, one can consider delaying it as long as there is no overflow occurring.
...
\begin{remark}
    The modular reduction is done by only performing subtractions and additions. It takes advantage of
...
For the modular sum over 64-bit integers, the speedup factor obtained by Mathemagix with AVX2 intrinsics is about 1.8
which is less than what we have measured. 
This can be explained by the evolution of the architectures as the difference in the throughput of the instruction
...
\paragraph{Scalar-vector multiplication} Given an integer $w \in \mathbb{Z}/n\mathbb{Z}$ and a vector $b$ of $N$ coefficients in $\mathbb{Z}/n\mathbb{Z}$, we compute
...
Like so, it requires performing a reduction for each binary operation.
...
In this case, we don't retrieve the high and low parts immediately. Instead,
we compute the sum of each $r_{lo}$, $r_{mi}$ and $r_{hi}$ computed like in section
...
\begin{remark}
    The size of the result is not a limit for the number of terms we can sum.
...
two processors, the versions with AVX are faster than FLINT's implementation for
integers of size 60 bits.
However, the Cascade Lake processor got no significant speedup over the sequential version
for the AVX2 version, and the timings for both AVX2 and AVX512 are very similar.

\section{Butterfly Fast Fourier Transform}

The butterfly refers to an operation that takes place in the Cooley-Tukey algorithm \cite{Cooley_Tukey_1965}, a common algorithm to perform 
a Fast Fourier Transform (FFT), which takes as inputs a modulus $n$ and a scalar $w$ of at most 64 bits, and two integers 
$x, y$ in $\mathbb{Z}/n\mathbb{Z}$, and performs the in-place computation:
\[
...
with the precomputation step. It allows saving some modular reductions compared to a basic implementation, if one is able to 
restrict the sizes of the modulus.

\bigskip
The algorithm 2 below shows our implementation of the Harvey's butterfly FFT.
...
in the context of an FFT.
It has not been done by us since it requires a further knowledge and analysis of the FFT algorithm
...
\begin{itemize}
    ...
    \item \texttt{dft4}: timings in seconds of the FFT without AVX vectorization from the pull request\footnote{\url{https://github.com/flintlib/flint/pull/2107}} that implements an integer-based 
    version of small prime FFT in FLINT (with a modulus up to 62 bits).    
...
This is not as well as Intel HEXL version whose speedup factor is around 2.7-2.8.
...
based on floating point numbers which is also very encouraging.
...
This is possible in the context of the FFT, and could have enhanced the butterfly FFT even more.
...
multiplications and additions but requires lowering the maximum bit size allowed to 52 which would need the
