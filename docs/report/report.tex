...
We will focus on the Intel R Advanced Vector Extensions sets for a Single Instruction Multiple Data (SIMD) vectorization,
and highlight their benefits and limitations using an advanced profiling function.

The outcomes of our study will be applied to a form of butterfly Fast Fourier Transform that we look forward to see it being merged
into FLINT.

\bigskip
Each of the studied operation have been implemented in C over the ring $\mathbb{Z}/n\mathbb{Z}$ with $n \in \mathbb{N}$.
...
\begin{tikzpicture}[very thick, black]
    ...
    {Intel(R) xeon(R) Gold 6354 \\ (Ice Lake, AVX512) \\ 3.0 GHz};
...
\begin{itemize}
    ...
    \item remarks on timings could be stated here ? (expect factor 4/8, estimate timings with clockspeed, memory issuess)
\end{itemize} }
...
They are separated in families allowing different operations on the basic types (int, long, float, double). We only used intrinsics from
...
For basic arithmetic operations, the modular reduction is among the most costly. 
Thus, to circumvent its high cost, one can try to delay it as long as there is no overflow occuring.
Another way is to modify the required sizes of the manipulated integers such that the reduction boils down to
...
\textcolor{blue}{
\begin{itemize}
    \item comment results: avx slightly better; would expect factor 4/8 but lot of load/store;
\end{itemize}}
...
\bigskip
These two operations share about the same properties and have both been implemented using the multiplication of 64-bit integers described 
...
\textcolor{blue}{
\begin{itemize}
    \item forward integration to FLINT
    \item case of special primes presented in \ref{app}
    \item IFMA ?
\end{itemize}
}
