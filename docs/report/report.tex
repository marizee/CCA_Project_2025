...
\bigskip
All he SIMD instructions have a name of the form:
\begin{center} 
...
\begin{example} % Remove if not necessary
\texttt{n} even, \texttt{a}, \texttt{b}, \texttt{r} arrays of size \texttt{n}.
\begin{lstlisting}
for(int i = 0; i < n; i++)
    r[i] = a[i] + b[i]
...
\end{example}
%With
%\texttt{-O3} optimization level, \texttt{gcc} can also unroll some loops,
%allowing it to automatically vectorize some loops.
...
C \texttt{\_\_attribute\_\_((...))}, this allows us to have a version with the
...
\paragraph{Scalar-vector multiplication} Given an integer $w \in \mathbb{Z}/n\mathbb{Z}$ vector $b$ of $N$ coefficients in $\mathbb{Z}/n\mathbb{Z}$, we compute
