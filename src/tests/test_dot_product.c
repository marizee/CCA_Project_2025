...
int main(int argc, char** argv) {
    ...
    start = clock();
    seq_dot_product(&res,vec1,vec2,len);
    end = clock();
    tseq = ((double) (end - start)) / CLOCKS_PER_SEC;
    printf("seq=\t\t%.5es\n", tseq);

    start = clock();
    seq_dot_product_vectorized(&res1,vec1,vec2,len);
    end = clock();
    tseqv = ((double) (end - start)) / CLOCKS_PER_SEC;
    printf("seqv=\t\t%.5es\n", tseqv);

    start = clock();
    seq_dot_product_unrolled(&res2,vec1,vec2,len);
    end = clock();
    tseq_unr = ((double) (end - start)) / CLOCKS_PER_SEC;
    printf("unr=\t\t%.5es\n", tseq_unr);

    start = clock();
    simd2_dot_product(&res3,vec1,vec2,len);
    end = clock();
