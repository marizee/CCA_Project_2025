...
int main(int argc, char** argv)
{
    ...
    for (len = 1; len < 200; ++len)
    {
        ...
        flint_printf("%d\t\t%.1lf\t\t%.1lf\t\t%.1lf\t\t%.1lf\t\t%.1lf", len,
                    (mins[len-1]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100),
                    (mins_vec[len-1]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100),
                    (mins_unrolled[len-1]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100),
                    (mins_simd2[len-1]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100),
                    (mins_simd2_unrolled[len-1]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100));
        #if defined(__AVX512F__)
        flint_printf("\t%3.f\t\t%3.f\n",
                    (mins_simd512[len-1]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100),
                    (mins_simd512_unrolled[len-1]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100));
        #else
        flint_printf("\n");
        #endif
    }
    ...
    for ( ; len <= 8000; len+=150)
    {
        ...
        flint_printf("%d\t\t%.1lf\t\t%.1lf\t\t%.1lf\t\t%.1lf\t\t%.1lf", len,
                    (mins[199 + (len-200)/150]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100),
                    (mins_vec[199 + (len-200)/150]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100),
                    (mins_unrolled[199 + (len-200)/150]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100),
                    (mins_simd2[199 + (len-200)/150]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100),
                    (mins_simd2_unrolled[199 + (len-200)/150]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100));
        #if defined(__AVX512F__)
        flint_printf("\t%3.f\t\t%3.f\n",
                    (mins_simd512[199 + (len-200)/150]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100),
                    (mins_simd512_unrolled[199 + (len-200)/150]/(double)FLINT_CLOCK_SCALE_FACTOR)/(len*100));
        #else
        flint_printf("\n");
        #endif
    }
    ...
    for (int i = 13; i < 23; i++)
    {
        ...
        flint_printf("%d\t\t%.1lf\t\t%.1lf\t\t%.1lf\t\t%.1lf\t\t%.1lf", info.length,
                    (mins[239+i]/(double)FLINT_CLOCK_SCALE_FACTOR)/(info.length*100),
                    (mins_vec[239+i]/(double)FLINT_CLOCK_SCALE_FACTOR)/(info.length*100),
                    (mins_unrolled[239+i]/(double)FLINT_CLOCK_SCALE_FACTOR)/(info.length*100),
                    (mins_simd2[239+i]/(double)FLINT_CLOCK_SCALE_FACTOR)/(info.length*100),
                    (mins_simd2_unrolled[239+i]/(double)FLINT_CLOCK_SCALE_FACTOR)/(info.length*100));
        #if defined(__AVX512F__)
        flint_printf("\t%3.f\t\t%3.f\n",
                    (mins_simd512[239+i]/(double)FLINT_CLOCK_SCALE_FACTOR)/(info.length*100),
                    (mins_simd512_unrolled[239+i]/(double)FLINT_CLOCK_SCALE_FACTOR)/(info.length*100));
        #else
        flint_printf("\n");
        #endif
